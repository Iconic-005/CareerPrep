...
// Rich Coding Question Repository covering all 12 Topics
export const CODING_QUESTIONS = [
  // Arrays
  {
    ...
    solution: 'Hash Map approach for O(N) time and O(N) space complexity.',
  ...
  // SQL
  {
    ...
    starterCode: {
      ...
      'JavaScript': `SELECT d.Name AS Department, e.Name AS Employee, e.Salary\nFROM Employee e JOIN Department d ON e.DepartmentId = d.Id;`
    },
...
export class CodingQuestionGeneratorService {
  ...
  static getQuestionByTopic(topic) {
    if (!topic || topic === 'All') return CODING_QUESTIONS;
    return CODING_QUESTIONS.filter(q => q.topic.toLowerCase() === topic.toLowerCase());
  }
